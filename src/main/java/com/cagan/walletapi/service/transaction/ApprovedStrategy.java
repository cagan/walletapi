...
@Slf4j
@Service
public class ApprovedStrategy implements TransactionApprovalStrategy {
    ...
    private void handleDeposit(Transaction transaction, Wallet wallet) {
        BigDecimal updatedUsableBalance = wallet.getUsableBalance().add(transaction.getAmount());
        wallet.setUsableBalance(updatedUsableBalance);
        log.info("Approved Deposit of {} has been processed for wallet {}", transaction.getAmount(), wallet.getId());
    }
    ...
    private void handleWithdraw(Transaction transaction, Wallet wallet) {
        ...
        log.info("Approved Withdrawal of {} has been processed for wallet {}", transaction.getAmount(), wallet.getId());
    }
